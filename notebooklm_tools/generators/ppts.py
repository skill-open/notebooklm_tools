...
async def process_ppt_batch(
    ...
    log_message("\n检查已存在的 PPT 文件...", log_file, "INFO")
    skipped_count = 0
    
    for task in tasks:
        safe_title = sanitize_filename(Path(task.source_title).stem)
        task.output_filename = f"{safe_title}_slides.pdf"
        output_path = output_dir / task.output_filename
        
        if output_path.exists():
            log_message(f"⊘ 跳过已存在的 PPT: {task.output_filename}", log_file, "INFO")
            task.status = "completed"
            task.skipped = True
            skipped_count += 1
    
    if skipped_count > 0:
        log_message(f"已跳过 {skipped_count} 个已处理的 PPT", log_file, "INFO")
    
    # 过滤出需要处理的任务
    pending_tasks = [t for t in tasks if t.status != "completed"]
    
    if not pending_tasks:
        log_message("所有 PPT 都已存在，无需重新生成", log_file, "INFO")
        return
...
async def main() -> None:
    ...
    # 创建任务列表
    tasks = [
        PPTGenerationTask(source_id=s["id"], source_title=s["title"])
