    ...
    try:
        ...
        if code == 0:
            ...
            if retry_count < MAX_DOWNLOAD_RETRIES:
                ...
                return await download_video(notebook_id, artifact_id, output_path, retry_count + 1)
            else:
        ...
        if retry_count < MAX_DOWNLOAD_RETRIES:
            print(f"  下载异常，{retry_count + 1}/{MAX_DOWNLOAD_RETRIES} 次重试中...")
...
async def process_video_batch(
    ...
    for task in tasks:
        ...
        if output_path.exists():
            log_message(f"⊘ 跳过已存在的视频：{task.output_filename}", log_file, "INFO")
            task.status = "completed"
            task.skipped = True
            skipped_count += 1
    
    if skipped_count > 0:
        log_message(f"已跳过 {skipped_count} 个已处理的视频", log_file, "INFO")
    
    # 过滤出需要处理的任务
    pending_tasks = [t for t in tasks if t.status != "completed"]
    
    if not pending_tasks:
        log_message("所有视频都已存在，无需重新生成", log_file, "INFO")
        return
